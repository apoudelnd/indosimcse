#!/bin/bash
#$ -pe smp 1      # Specify parallel environment and legal core size
#$ -q gpu             # Specify queue
#$ -l gpu_card=1  # number of GPU
#$ -N test__      # cchit_scibert_completion

echo "Unsupervised finetune"
proj_root="/afs/crc.nd.edu/user/a/apoudel/projects/indosimcse"
if [ ! -d $proj_root ]; then
    if [ $(basename $PWD) == "indosimcse" ]; then
        # When $PWD is project root
        proj_root=$PWD
    else
        # When $PWD is current direcotry
        proj_root=$(realpath $PWD/../../../../../)
    fi
fi
echo "project root:$proj_root"
module load python/3.7.3
cd $proj_root
source ${proj_root}/venv3.7/bin/activate

seed=${1:-12}
echo "seed:$seed"

cur_script_dir="${proj_root}/clip/"
py_file="${proj_root}/clip/train.py"
data_folder="${proj_root}/data/output_test.csv"
img_pretrained_model='clip-ViT-B-32'
text_pretrained_model='sentence-transformers/clip-ViT-B-32-multilingual-v1'
cur_time=$(date +"%Y_%m_%d_%I_%M_%S")
output_dir="${cur_script_dir}/output/${cur_time}_${seed}"
log_dir="${cur_script_dir}/logs/${cur_time}_${seed}"
image_dir="${cur_script_dir}/images/${cur_time}_${seed}"


# img_pretrained_model = 'clip-ViT-B-32'
# text_pretrained_model = 'sentence-transformers/clip-ViT-B-32-multilingual-v1'
# mkdir -p $output_dir
# mkdir -p $log_dir
# mkdir -p $image_dir

python $py_file \
    --image_model_path $img_pretrained_model \
    --text_model_path $text_pretrained_model \
    --data_folder $data_folder \
    --reduction_teq 'UMAP' \
    --output_dir $output_dir \
    --batch_size  64 \
    --image_dir $image_dir \
    --max_seq_length 256 \
    "$@"
